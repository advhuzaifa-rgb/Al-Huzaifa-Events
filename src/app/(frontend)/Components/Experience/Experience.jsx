...
<path fillRule="evenodd" clipRule="evenodd" d="M9.04139 11.625C8.78596 11.6251 8.53629 11.7008 8.32392 11.8428C8.11156 11.9847 7.94605 12.1864 7.84831 12.4224C7.75057 12.6584 7.72499 12.9181 7.7748 13.1686C7.82462 13.4191 7.9476 13.6492 8.12818 13.8299L14.5865 20.2882C14.8287 20.5304 15.1572 20.6664 15.4997 20.6664C15.8422 20.6664 16.1707 20.5304 16.4129 20.2882L22.8713 13.8299C23.0519 13.6492 23.1748 13.4191 23.2246 13.1686C23.2745 12.9181 23.2489 12.6584 23.1511 12.4224C23.0534 12.1864 22.8879 11.9847 22.6755 11.8428C22.4632 11.7008 22.2135 11.6251 21.9581 11.625H9.04139Z" fill="#CD7D6D"/>
</svg>

)

// Placeholder icon — the SVG shared for this actually renders as the word "RSVP",
// not an arrow. Swap this out once the real arrow asset is provided.
const RsvpArrow = () => (
  <svg width="18" height="14" viewBox="0 0 18 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    />
    <path d="M17 7H1" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
)
...
const Experience = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('al-huzaifa:open-experiences', handleOpenRequest)
    return () => window.removeEventListener('al-huzaifa:open-experiences', handleOpenRequest)
  }, [])

  const handleRsvpClick = () => {
    setOpen(false)
    window.dispatchEvent(new CustomEvent('al-huzaifa:open-rsvp'))
  }

  return (
    <div className={styles.main} id="experience" ref={mainRef}>
                        ...
                        {events.map((event) => (
                          <div className={styles.event} key={event.title}>
                            ...
                            {event.cta ? (
                              ...
                              >
                                <span className={styles.eventCtaText}>RSVP</span>
                                <span className={styles.eventCtaArrow}>
                                  <RsvpArrow />
                                </span>
                              </button>
                            ) : (
